...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    @keyframes glow {
      0% { box-shadow: 0 0 15px rgba(74,222,128,0.4); border-color: rgba(74,222,128,0.5); }
      100% { box-shadow: none; border-color: #1e1e2e; }
    }
    @keyframes slideIn {
    ...
    @keyframes countUp {
      from { transform: scale(1.3); color: #4ade80; }
    ...
    .stat-card .delta.show { opacity: 1; }
    ...
    .rate-bar .speed { color: #4ade80; font-weight: 600; }
    .rate-bar .sep { color: #333; }
    ...
    .feed-header h2 { font-size: 16px; color: #ccc; }
    .feed {
      background: #12121a;
      border: 1px solid #1e1e2e;
      border-radius: 12px;
    ...
    .feed-row {
      display: grid;
      grid-template-columns: 28px 1.2fr 0.8fr 1fr 0.8fr 60px;
      gap: 8px;
      padding: 8px 16px;
    ...
    .feed-row .photo { font-size: 14px; text-align: center; }
    .feed-row .nome { color: #fff; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .feed-row .field { color: #444; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .feed-row .field.filled { color: #4ade80; }
    .feed-row .time { color: #555; text-align: right; font-size: 11px; }
    .feed-row-header {
      display: grid;
      grid-template-columns: 28px 1.2fr 0.8fr 1fr 0.8fr 60px;
      gap: 8px;
...
<body>
  <div class="header">
    ...
    <div class="sub">Phase B — UFC.com Bio Scraping</div>
    <div class="live"><span class="dot"></span> LIVE — atualizando a cada 2s</div>
  </div>

  <div class="done-banner" id="done">✅ Scraper finalizado! Nenhuma atualização no último minuto.</div>
  ...
  <script>
    ...
    async function update() {
      try {
        ...
        if (perMin === 0) {
          zeroCount++;
          if (zeroCount > 5) document.getElementById('done').style.display = 'block';
        } else {
          zeroCount = 0;
        ...
        statsEl.innerHTML = FIELDS.map(f => {
          ...
          const glowClass = delta > 0 ? 'glow' : '';
          const bumpClass = delta > 0 ? 'bumped' : '';
          prevValues[f.key] = val;
          return '<div class="stat-card ' + glowClass + '">' +
            '<div class="label">' + f.label + '</div>' +
            '<div class="value ' + bumpClass + '">' + val.toLocaleString() + '</div>' +
        ...
        const eta = avgPerMin > 0 ? Math.ceil(remaining / avgPerMin) : '∞';
        rateEl.innerHTML = 
          ...
          '<span class="sep">|</span>' +
          '<span>ETA: <span class="speed">~' + eta + ' min</span></span>' +
          '<span class="sep">|</span>' +
        ...
        if (recent.length === 0) {
          feedEl.innerHTML = '<div class="empty">Nenhuma atualização no último minuto — scraper pode ter finalizado ou pausado</div>';
        } else {
          let html = '<div class="feed-row-header">' +
            '<span></span><span>Nome</span><span>País</span><span>Academia</span><span>Categoria</span><span>Tempo</span>' +
          '</div>';
          ...
          for (const r of recent) {
            ...
            html += '<div class="feed-row">' +
              ...
              '<span class="field ' + (r.academia ? 'filled' : '') + '">' + (r.academia ? r.academia.slice(0,20) : '—') + '</span>' +
